...
async def restart_gen_handler(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(
        ...
        f"✍️ Напишите новый промпт (и/или прикрепите до 10 фото в качестве референсов).",
...
async def set_style(cb: CallbackQuery, state: FSMContext):
    ...
    text = (
        ...
        "✍️ Все готово! Теперь просто <b>напишите промпт</b> (описание картинки) и/или прикрепите <b>до 10 фото</b> в качестве референсов."
    )
